...
var UIColorPicker = (function UIColorPicker() {
	...
	Color.prototype.setHue = function setHue(value) {
		if (typeof (value) !== 'number' || isNaN(value) === true || value < 0
				|| value > 359)
			return;
		this.hue = value;
		this.updateRGB();
	...
	Color.prototype.setHexa = function setHexa(value) {
		...
		if (value.length === 3)
			value = value.replace(/([0-9A-F])([0-9A-F])([0-9A-F])/i,
					'$1$1$2$2$3$3');
	...
	Color.prototype.getHSLA = function getHSLA() {
		...
		var a = '';
		var v = '';
		var hsl = '(' + this.hue + ', ' + this.saturation + '%, '
				+ this.lightness + '%';
		var x = parseFloat(this.a);
		if (x !== 1) {
			a = 'a';
	...
	function ColorPicker(node) {
		...
		this.newInputComponent('S', 'saturation', this.inputChangeSaturation
				.bind(this));
		this.newInputComponent('V', 'value', this.inputChangeValue.bind(this));
		this.newInputComponent('L', 'lightness', this.inputChangeLightness
				.bind(this));
		...
		this.newInputComponent('alpha', 'alpha', this.inputChangeAlpha
				.bind(this));
	...
	ColorPicker.prototype.newInputComponent = function newInputComponent(title,
			topic, onChangeFunc) {
	...
	ColorPicker.prototype.inputChangeThema = function inputChangeThema(e) {
...
var ColorPickerTool = (function ColorPickerTool() {
	...
	var ColorPalette = (function ColorPalette() {
		...
		ColorSample.prototype.updateHue = function updateHue(color, degree,
				steps) {
			this.color.copy(color);
		...
		ColorSample.prototype.updateLightness = function updateLightness(color,
				value, steps) {
		...
		var Palette_t = function Palette_t(text, size) {
			this.samples = [];
			this.locked = false;
		...
		var init = function init() {
			...
			color_palette_t = getElemById('Thema_colors');
			createHuePalette();
	...
	/* ===================================================================================== */
	/* ===================================================================================== */
	var ColorPickerSamples_t = (function ColorPickerSamples_t() {
		...
		$.getJSON("resources/json/test.json", function(data) {
			colorNames = data;
		});
		ColorSample.prototype.nameConverter = function nameConverter(r, g, b) {
			var minest = 500;
			var crrectColor = {};
			for (i = 0; i < colorNames.length; i++) {
				var R = colorNames[i]["R"];
				var G = colorNames[i]["G"];
				var B = colorNames[i]["B"];
				var distance = (r - R) * (r - R) + (g - G) * (g - G) + (b - B)
						* (b - B);
				distance = Math.sqrt(distance);
				if (minest > distance) {
					minest = distance;
					crrectColor = colorNames[i];
				}
			}
			console.log(r+" "+g+" "+b);
			console.log(crrectColor);
			return crrectColor;
		}
		...
		ColorSample.prototype.dragStart = function dragStart(e) {
			e.dataTransfer.setData('sampleID', this.uid);
			e.dataTransfer.setData('location', 'Thema_pick_colors');
		...
		var AddSampleButton = (function AddSampleButton() {
			...
			var activeAjax = function activeAjax() {
				var sample = new ColorSample();
				container.appendChild(sample.node);
				updatePosition(_index + 1);
				updateUI();
		...
		var init = function init() {
			container = getElemById('Thema_pick_colors');
			trash_can = getElemById('trash-can');
	...
	var CanvasSamples = (function CanvasSamples() {
		...
		CanvasSample.prototype.updatePosition = function updatePosition(posX,
				posY) {
		...
		var setActiveSample = function setActiveSample(sample) {
			...
			ColorPickerSamples_t.unsetActiveSample();
			Tool.unsetVoidSample();
			unsetActiveSample();
			active = sample;
			active.activate();
	...
	var Tool = (function Tool() {
		...
		var getSampleColorFrom = function getSampleColorFrom(e) {
			...
			if (location === 'Thema_pick_colors')
				return ColorPickerSamples_t.getSampleColor(sampleID);
